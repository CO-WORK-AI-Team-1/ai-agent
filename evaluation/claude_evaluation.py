...
from llm.claude_client import generate_claude_response
...
def main():
    print("Claude LLM Evaluation 시작\n")
    ...
    for index, test in enumerate(TEST_CASES, start=1):
        ...
        try:
            ...
            response = generate_claude_response(test["prompt"])
    ...
    print(f"실패: {failure_count}/{len(TEST_CASES)}")

    if success_count > 0:
        average_time = total_time / success_count
        print(f"성공 요청 평균 응답 시간: {average_time:.2f}초")
    else:
